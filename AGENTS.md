...
- Map_S03B 只是实现最终目标的测试场景之一。增加、删除或调整其中的模型、灯光、材质、参数和相机，
  不得改变 Zevy 的通用开发路线，也不得被当成产品架构前提。
- 只对某个 Map 名称、Actor 名称、固定灯数、固定坐标或当前构图有效的优化只能作为诊断夹具或可证伪实验；
  如果不能转化为通用引擎组件、数据模型、算法、资产元数据或可复用配置，该优化按项目标准视为无效。
- `is_map_*`、硬编码实体集合和场景专属 tuning 必须在检查点中明确标记为测试 harness/技术债，不能冒充
  renderer feature。实验胜出后必须把机制下沉到通用 pipeline，并删除产品路径对测试 Map 的依赖。
- 通用化验收至少要覆盖第二个独立场景或合成压力场景，并覆盖不同运动类型、灯数、空间尺度和遮挡关系；
  Map_S03B 的单场景视觉或性能结果不能单独决定全局默认值。
- 场景调整可以用于放大问题、建立基准和构造反例，但不能用降低该场景内容复杂度来替代算法突破。
