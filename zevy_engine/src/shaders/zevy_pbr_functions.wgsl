// Derived from Bevy 0.16.1 `bevy_pbr/render/pbr_functions.wgsl`
// (dual-licensed MIT OR Apache-2.0). Keep this file in sync when Bevy changes
// its StandardMaterial bindings or PBR helper interfaces.
...
// Zevy scalable local-lighting experiment. These two constants are replaced
// from RenderQualityConfig before Bevy extracts the shader to the render world.
// The strongest local lights at each shading point are deterministic Heroes.
// The remaining local-light tail uses a fixed, importance-sampled shadowed
...
// World-space sampling keeps the stochastic decision correlated between the
// two XR eyes. A 12.5 cm cell avoids screen-space noise swimming independently
// in each eye, while the low-frequency epoch lets every tail light contribute
// over time without changing the lighting selection every frame.
fn zevy_light_sample_jitter(world_position: vec3<f32>) -> f32 {
...
fn zevy_point_light_has_shadow(light_id: u32) -> bool {
    return (view_bindings::clusterable_objects.data[light_id].flags &
...
// Cheap upper-bound proxy for the light's likely contribution at this shading
// point. It deliberately excludes the material BRDF: evaluating that is the
// expensive operation this sampler is trying to avoid. Luminance multiplied by
// Bevy's range attenuation gives a useful PDF for diffuse and rough surfaces.
fn zevy_point_light_importance(light_id: u32, world_position: vec3<f32>) -> f32 {
    let light = &view_bindings::clusterable_objects.data[light_id];
    let light_to_fragment = (*light).position_radius.xyz - world_position;
    let distance_squared = dot(light_to_fragment, light_to_fragment);
    let range_factor = distance_squared * (*light).color_inverse_square_range.w;
    let smooth_factor = clamp(1.0 - range_factor * range_factor, 0.0, 1.0);
    let attenuation = smooth_factor * smooth_factor / max(distance_squared, 0.0001);
    let luminance = dot(
...
fn apply_pbr_lighting(
    ...
    if (point_light_count <= full_evaluation_budget) {
        ...
        for (var hero_index = 0u;
                hero_index < ZEVY_POINT_LIGHT_HERO_SAMPLES;
                hero_index = hero_index + 1u) {
            var light_id = hero_light_0;
            if (hero_index == 1u) {
                light_id = hero_light_1;
            }
            if (light_id != 0xffffffffu) {
                ...
                direct_light += lighting::point_light(
                    light_id, &lighting_input, enable_diffuse
                ) * shadow;
            }
        }
        ...
        } else if (tail_sample_count > 0u && tail_importance_sum > 0.0) {
...
#ifdef STANDARD_MATERIAL_DIFFUSE_TRANSMISSION
    // Transmission is rare and visually sensitive. Keep its local lighting
    // deterministic until it gets a dedicated stochastic reconstruction path.
    for (var i = first_point_light; i < point_light_end; i = i + 1u) {
        let light_id = clustering::get_clusterable_object_id(i);
#ifdef LIGHTMAP
        let enable_diffuse =
        ...
        transmitted_light += lighting::point_light(
            light_id,
