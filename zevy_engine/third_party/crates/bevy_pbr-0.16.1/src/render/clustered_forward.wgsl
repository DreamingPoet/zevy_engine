...
// Zevy fixed-budget PointLight selection attached to a cluster header. The
// CPU selection is shared by a Cyclopean XR supercluster; fragment shaders can
// therefore shade four IDs directly instead of scanning every candidate.
// Uniform-buffer platforms preserve Bevy's original packed ABI and return an
// invalid selection so the caller can use its reference path.
struct PreselectedClusterPointLights {
    light_ids: vec4<u32>,
    estimator_weights: vec4<f32>,
}

fn unpack_preselected_point_lights(cluster_index: u32) -> PreselectedClusterPointLights {
#if AVAILABLE_STORAGE_BUFFER_BINDINGS >= 3
    return PreselectedClusterPointLights(
        bindings::cluster_offsets_and_counts.data[cluster_index][2],
        bitcast<vec4<f32>>(bindings::cluster_offsets_and_counts.data[cluster_index][3]),
    );
#else
    return PreselectedClusterPointLights(
