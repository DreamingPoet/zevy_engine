...
/// Number of fixed-budget PointLight entries Zevy can attach to a cluster.
///
/// The entries are stored in spare words of the storage-buffer cluster header,
/// so they add no bind group and preserve the original clustered object list.
/// Uniform-buffer platforms leave them empty and use the normal shader path.
pub const PRESELECTED_CLUSTER_POINT_LIGHTS: usize = 4;
...
pub struct VisibleClusterableObjects {
    ...
    preselected_point_lights: [Option<Entity>; PRESELECTED_CLUSTER_POINT_LIGHTS],
    preselected_point_light_weights: [f32; PRESELECTED_CLUSTER_POINT_LIGHTS],
}
...
struct GpuClusterOffsetsAndCountsStorage {
    ...
    #[size(runtime)]
    /// Entries 0 and 1 retain Bevy's original offset/count ABI. Entry 2 stores
    /// four preselected PointLight indices and entry 3 stores their f32
    /// estimator weights bitcast to u32.
    data: Vec<[UVec4; 4]>,
}
...
impl Clusters {
    ...
    /// Returns the number of clusters in X, Y, and Z for this view.
    #[inline]
    pub fn dimensions(&self) -> UVec3 {
        self.dimensions
    }

    /// Returns the total number of allocated clusters for this view.
    #[inline]
    pub fn len(&self) -> usize {
        self.clusterable_objects.len()
    }

    /// Returns true when this view currently has no allocated clusters.
    #[inline]
    pub fn is_empty(&self) -> bool {
        self.clusterable_objects.is_empty()
    }

    /// Returns the PointLight entities affecting one cluster.
    pub fn point_light_entities(&self, cluster_index: usize) -> &[Entity] {
        let Some(cluster) = self.clusterable_objects.get(cluster_index) else {
            return &[];
        };
        &cluster.entities[..cluster.counts.point_lights as usize]
    }

    /// Computes the view-space AABB represented by one cluster.
    ///
    /// This is exposed for renderer extensions that select a fixed light set at
    /// a representative froxel position. It uses the same geometry as Bevy's
    /// light-volume assignment.
    pub fn cluster_aabb(&self, camera: &Camera, cluster_index: usize) -> Option<Aabb> {
        if cluster_index >= self.clusterable_objects.len() || self.dimensions == UVec3::ZERO {
            return None;
        }
        let screen_size = camera.physical_viewport_size()?;
        let z = cluster_index as u32 % self.dimensions.z;
        let xy = cluster_index as u32 / self.dimensions.z;
        let x = xy % self.dimensions.x;
        let y = xy / self.dimensions.x;
        let clip_from_view = camera.clip_from_view();
        Some(assign::compute_aabb_for_cluster(
            self.near,
            self.far,
            self.tile_size.as_vec2(),
            screen_size.as_vec2(),
            clip_from_view.inverse(),
            clip_from_view.w_axis.w == 1.0,
            self.dimensions,
            UVec3::new(x, y, z),
        ))
    }

    /// Stores a fixed-budget PointLight selection and estimator weights for a
    /// cluster. `None` entries are encoded as invalid IDs in the GPU header.
    pub fn set_preselected_point_lights(
        &mut self,
        cluster_index: usize,
        selected: [Option<(Entity, f32)>; PRESELECTED_CLUSTER_POINT_LIGHTS],
    ) -> bool {
        let Some(cluster) = self.clusterable_objects.get_mut(cluster_index) else {
            return false;
        };
        for (slot, entry) in selected.into_iter().enumerate() {
            if let Some((entity, weight)) = entry {
                cluster.preselected_point_lights[slot] = Some(entity);
                cluster.preselected_point_light_weights[slot] = weight.max(0.0);
            } else {
                cluster.preselected_point_lights[slot] = None;
                cluster.preselected_point_light_weights[slot] = 0.0;
            }
        }
        true
    }
...
pub fn extract_clusters(
    ...
    for (entity, clusters, camera) in &views {
        ...
        for cluster_objects in &clusters.clusterable_objects {
            let preselected_point_lights = core::array::from_fn(|slot| {
                cluster_objects.preselected_point_lights[slot]
                    .and_then(|entity| mapper.get(entity).ok())
            });
            data.push(ExtractedClusterableObjectElement::ClusterHeader {
                counts: cluster_objects.counts,
                preselected_point_lights,
                preselected_point_light_weights: cluster_objects
                    .preselected_point_light_weights,
            });
        ...
        for record in &extracted_clusters.data {
            match record {
                ExtractedClusterableObjectElement::ClusterHeader {
                    ...
                    let preselected_indices = core::array::from_fn(|slot| {
                        preselected_point_lights[slot].and_then(|entity| {
...
impl ViewClusterBindings {
    ...
    fn push_offset_and_counts(
        &mut self,
        offset: usize,
        counts: &ClusterableObjectCounts,
        preselected_point_lights: [Option<usize>; PRESELECTED_CLUSTER_POINT_LIGHTS],
        preselected_point_light_weights: [f32; PRESELECTED_CLUSTER_POINT_LIGHTS],
    ) {
        match &mut self.buffers {
            ViewClusterBuffers::Uniform {
            ...
            ViewClusterBuffers::Storage {
                ...
                cluster_offsets_and_counts.get_mut().data.push([
                    ...
                    UVec4::from_array(preselected_point_lights.map(|index| {
                        index.map_or(u32::MAX, |index| index as u32)
                    })),
                    UVec4::from_array(preselected_point_light_weights.map(f32::to_bits)),
                ]);
            }
        }
