...
fn log_frame_step(counter: &mut Local<u32>, message: &str) {
    if **counter < 8 {
        info!("{message}");
        **counter += 1;
    }
...
pub fn wait_frame(
    ...
    mut commands: Commands,
    mut log_count: Local<u32>,
) {
    log_frame_step(&mut log_count, "XR frame step: wait_frame begin");
    let state = frame_waiter.wait().expect("Failed to wait frame");
    log_frame_step(&mut log_count, "XR frame step: wait_frame done");
...
pub fn insert_texture_views(
    ...
    log_frame_step(&mut log_count, "XR frame step: acquire_image begin");
    let index = swapchain.acquire_image().expect("Failed to acquire image");
    log_frame_step(&mut log_count, "XR frame step: acquire_image done");
...
pub fn wait_image(mut swapchain: ResMut<OxrSwapchain>, mut log_count: Local<u32>) {
    log_frame_step(&mut log_count, "XR frame step: wait_image begin");
    swapchain
        .wait_image(openxr::Duration::INFINITE)
        .expect("Failed to wait image");
    log_frame_step(&mut log_count, "XR frame step: wait_image done");
}
...
pub fn begin_frame(mut frame_stream: ResMut<OxrFrameStream>, mut log_count: Local<u32>) {
    log_frame_step(&mut log_count, "XR frame step: begin_frame begin");
    frame_stream.begin().expect("Failed to begin frame");
    log_frame_step(&mut log_count, "XR frame step: begin_frame done");
}
...
pub fn release_image(mut swapchain: ResMut<OxrSwapchain>, mut log_count: Local<u32>) {
    ...
    log_frame_step(&mut log_count, "XR frame step: release_image done");
}
...
pub fn end_frame(world: &mut World) {
    ...
    world.resource_scope::<OxrFrameStream, ()>(|world, mut frame_stream| {
        ...
        let log_index = END_FRAME_LOG_COUNT.fetch_add(1, Ordering::Relaxed);
        if log_index < 8 {
            info!(
                "XR frame step: end_frame begin should_render={} layers={}",
        ...
        if let Err(e) = frame_stream.end(
            ...
            info!("XR frame step: end_frame done");
        }
    });
}
