        ...
        .expect("Failed to poll event")
    {
        match &event {
            Event::SessionStateChanged(state) => {
                info!("polled OpenXR event: SessionStateChanged({:?})", state.state());
            }
            Event::EventsLost(lost) => {
                warn!(
                    "polled OpenXR event: EventsLost({})",
                    lost.lost_event_count()
                );
            }
            Event::InstanceLossPending(_) => info!("polled OpenXR event: InstanceLossPending"),
            Event::InteractionProfileChanged(_) => {
                info!("polled OpenXR event: InteractionProfileChanged");
            }
            _ => info!("polled OpenXR event: other"),
        }
