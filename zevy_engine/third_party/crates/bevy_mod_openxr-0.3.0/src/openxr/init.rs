...
fn init_xr_session(
    ...
    let candidate_formats = if let Some(formats) = &formats {
        formats
            .iter()
            .copied()
            .filter(|wanted_format| available_formats.contains(wanted_format))
            .collect::<Vec<_>>()
    } else {
        available_formats.clone()
    };

    let mut selected_swapchain = None;
    for format in candidate_formats {
        match session.create_swapchain(SwapchainCreateInfo {
            ...
            Ok(swapchain) => {
                info!("OpenXR swapchain created with format {format:?}");
                selected_swapchain = Some((format, swapchain));
                break;
            }
            Err(err) => {
                warn!("OpenXR swapchain format {format:?} failed: {err}");
            }
        }
    }

    let (format, swapchain) = selected_swapchain.ok_or(OxrError::NoAvailableFormat)?;
