...
unsafe impl GraphicsExt for openxr::Vulkan {
    ...
    fn init_graphics(
        ...
        let vk_instance = unsafe {
            let xr_instance_extensions = instance.vulkan_legacy_instance_extensions(system_id)?;
            let xr_instance_extension_names = xr_instance_extensions
                .split_whitespace()
                .map(CString::new)
                .collect::<std::result::Result<Vec<_>, _>>()?;
            let mut extensions_cchar: Vec<_> = extensions.iter().map(|s| s.as_ptr()).collect();
            extensions_cchar.extend(xr_instance_extension_names.iter().map(|s| s.as_ptr()));
            ...
            #[cfg(target_os = "android")]
            let vk_instance = vk_entry.create_instance(&instance_create_info, None)?;
        ...
        info!("Requesting OpenXR Vulkan physical device");
        ...
        info!("OpenXR Vulkan physical device acquired");
        ...
        let (wgpu_open_device, vk_device_ptr, queue_family_index) = {
            let xr_device_extensions = instance.vulkan_legacy_device_extensions(system_id)?;
            let xr_device_extension_names = xr_device_extensions
                .split_whitespace()
                .map(CString::new)
                .collect::<std::result::Result<Vec<_>, _>>()?;
            let mut extensions_cchar: Vec<_> =
                device_extensions.iter().map(|s| s.as_ptr()).collect();
            extensions_cchar.extend(xr_device_extension_names.iter().map(|s| s.as_ptr()));
            ...
            let vk_device = unsafe {
                #[cfg(target_os = "android")]
                let vk_device = vk_instance.create_device(vk_physical_device, &info, None)?;
